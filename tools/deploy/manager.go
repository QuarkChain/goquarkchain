...
func (t *ToolManager) MakeClusterExe() {
	for _, v := range t.SSHSession {
		...
		v.RunCmd("docker exec -itd bjqkc /bin/bash -c  'cd /root/go/src/github.com/Quarkchain/goquarkchain && git pull && git checkout  remotes/origin/users/chunfengSun/fix-ipv4'")                                //go build 		//go build
