...
func TestPoSWOnRootChain(t *testing.T) {
	...
	for i := range cfglist {
		cfglist[i].Quarkchain.RootChainPoSWContractBytecodeHash = crypto.Keccak256Hash(contractCode)
		...
		poswConfig := cfglist[i].Quarkchain.Root.PoSWConfig
		...
		balance := map[string]*big.Int{cfglist[i].Quarkchain.GenesisToken: big.NewInt(100000000)}
