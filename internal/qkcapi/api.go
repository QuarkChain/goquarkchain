...
func (p *PublicBlockChainAPI) GetTransactionCount(address account.Address, blockNr *rpc.BlockNumber) (*hexutil.Big, error) {
	...
	return (*hexutil.Big)(big.NewInt(int64(data.TransactionCount))), nil
}
...
func (p *PublicBlockChainAPI) GetBalances(address account.Address, blockNr *rpc.BlockNumber) (map[string]interface{}, error) {
	...
	branch := account.NewBranch(data.Branch)
...
func (p *PublicBlockChainAPI) GetAccountData(address account.Address, blockNr *rpc.BlockNumber, includeShards *bool) (map[string]interface{}, error) {
	...
	if !*includeShards {
		...
		primary := map[string]interface{}{
			...
			"transactionCount": (*hexutil.Big)(big.NewInt(int64(data.TransactionCount))),
	...
	var (
		branchToAccountBranchData map[uint32]*qkcRPC.AccountBranchData
		err                       error
	)
	if blockNr == nil {
	...
	for value, data := range branchToAccountBranchData {
		branch := account.NewBranch(value)
...
func (p *PublicBlockChainAPI) GetWork(fullShardKey hexutil.Uint) []*hexutil.Big {
	...
	value := p.b.GetClusterConfig().Quarkchain.GetFullShardIdByFullShardKey(uint32(fullShardKey))
	branch := account.NewBranch(value)
	res, err := p.b.GetWork(&branch)
	if err != nil {
		log.Error("failed to get work", "err", err)
		return []*hexutil.Big{}
	}
