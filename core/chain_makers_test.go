...
func ExampleGenerateMinorBlockChain() {
	...
	chain, _ := GenerateMinorBlockChain(params.TestChainConfig, fakeClusterConfig.Quarkchain, genesis, new(consensus.FakeEngine), db, 5, func(config *config.QuarkChainConfig, i int, gen *MinorBlockGen) {
		...
		case 0:
			// In block 1, addr1 sends addr2 some ether.
			tx, _ := types.SignTx(types.NewEvmTransaction(gen.TxNonce(addr1.Recipient), account.BytesToIdentityRecipient(addr2.Recipient.Bytes()), big.NewInt(100000), params.TxGas, nil, 0, 0, 3, 0, nil, genesisTokenID, genesisTokenID), types.NewEIP155Signer(0, 0), prvKey1)
			gen.AddTx(config, transEvmTxToTx(tx))
		case 1:
			// In block 2, addr1 sends some more ether to addr2.
			// addr2 passes it on to addr3.
			tx1, _ := types.SignTx(types.NewEvmTransaction(gen.TxNonce(addr1.Recipient), account.BytesToIdentityRecipient(addr2.Recipient.Bytes()), big.NewInt(1000), params.TxGas, nil, 0, 0, 3, 0, nil, genesisTokenID, genesisTokenID), types.NewEIP155Signer(0, 0), prvKey1)
			tx2, _ := types.SignTx(types.NewEvmTransaction(gen.TxNonce(addr2.Recipient), account.BytesToIdentityRecipient(addr3.Recipient.Bytes()), big.NewInt(1000), params.TxGas, nil, 0, 0, 3, 0, nil, genesisTokenID, genesisTokenID), types.NewEIP155Signer(0, 0), prvKey2)
