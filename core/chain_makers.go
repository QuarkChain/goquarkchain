...
func (b *MinorBlockGen) AddTx(tx *types.Transaction) {
...
func (b *MinorBlockGen) AddTxWithChain(bc *MinorBlockChain, tx *types.Transaction) {
	if b.gasPool == nil {
...
// a similar non-validating proof of work implementation.
func GenerateMinorBlockChain(config *params.ChainConfig, parent *types.MinorBlock, engine consensus.Engine, db ethdb.Database, n int, gen func(int, *MinorBlockGen)) ([]*types.MinorBlock, []types.Receipts) {
	if config == nil {
	...
	genblock := func(i int, parent *types.MinorBlock, statedb *state.StateDB) (*types.MinorBlock, types.Receipts) {
		...
		rootHash, err := statedb.Commit(config.IsEIP158(new(big.Int).SetUint64(block.Header().Number)))
		if err != nil {
			panic(fmt.Sprintf("state write error: %v", err))
		}
		if err := statedb.Database().TrieDB().Commit(rootHash, false); err != nil {
			panic(fmt.Sprintf("trie write error: %v", err))
		}
		block.Finalize(b.receipts, rootHash,statedb.GetGasUsed(),statedb.GetXShardReceiveGasUsed(),statedb.GetXShardReceiveGasUsed(),nil)
...
func makeHeader(parent *types.MinorBlock, state *state.StateDB, difficulty *big.Int) *types.MinorBlockHeader {
	var time uint64 = parent.Time() + 40
	gasLimit := &serialize.Uint256{Value: new(big.Int).SetUint64(CalcGasLimit(parent, parent.GasLimit().Uint64(), parent.GasLimit().Uint64()))}
...
func makeBlockChain(parent *types.MinorBlock, n int, engine consensus.Engine, db ethdb.Database, seed int) []*types.MinorBlock {
	blocks, _ := GenerateMinorBlockChain(params.TestChainConfig, parent, engine, db, n, func(i int, b *MinorBlockGen) {
		b.SetCoinbase(account.Address{Recipient: account.Recipient{0: byte(seed), 19: byte(i)}, FullShardKey: 0})
	})
	return blocks
}
