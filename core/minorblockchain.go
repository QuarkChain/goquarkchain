...
func (m *MinorBlockChain) insertChain(chain []types.IBlock, verifySeals bool, force bool) (int, []interface{}, []*types.Log, [][]*types.CrossShardTransactionDeposit, error) {
...
func (m *MinorBlockChain) insertSidechain(it *insertIterator, force bool) (int, []interface{}, []*types.Log, [][]*types.CrossShardTransactionDeposit, error) {
	var (
		current      = m.CurrentBlock().NumberU64()
	...
	for i := len(hashes) - 1; i >= 0; i-- {
		...
		if len(blocks) >= 2048 || memory > 64*1024*1024 {
			...
			if _, _, _, _, err := m.insertChain(blocks, false, force); err != nil {
	...
	if len(blocks) > 0 {
		...
		return m.insertChain(blocks, false, force)
	}
	return 0, nil, nil, nil, nil
}
