...
type MinorEpochDataCache struct {
...
type MinorBlockChain struct {
	...
	epochDataCache        map[uint64]*MinorEpochDataCache
...
func NewMinorBlockChain(
	...
	bc := &MinorBlockChain{
		...
		epochDataCache:           make(map[uint64]*MinorEpochDataCache),
		engine:                   engine,
		vmConfig:                 vmConfig,
...
func (m *MinorBlockChain) GetAdjustedDifficulty(header types.IHeader) (*big.Int, uint64, error) {
	...
	if m.posw.IsPoSWEnabled(header.GetTime(), header.NumberU64()) {
		...
		stakePreBlock := m.GetStakePerBlock(header.NumberU64(), header.GetTime())
