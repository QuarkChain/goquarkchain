...
func (m *MinorBlockChain) setHead(head uint64) error {
	...
	// Rewind the header chain. We preserve block bodies so that reRunBlockWithState
	// can replay them to rebuild state trie after a crash. Only the commit marker
	// and canonical-hash mapping are removed; receipts are also discarded since
	// they would be regenerated by the replay anyway.
	batch := m.db.NewBatch()
