...
func TestShardStateRecoveryFromRootBlock(t *testing.T) {
	...
	fullShardID := env.clusterConfig.Quarkchain.Chains[1].ChainID<<16 | env.clusterConfig.Quarkchain.Chains[1].ShardSize | 0
	recoveredState, err := NewMinorBlockChain(env.db, nil, ethParams.TestChainConfig, env.clusterConfig, Engine, vm.Config{}, nil, fullShardID)
	// forks are pruned
...
func TestShardStateTecoveryFromGenesis(t *testing.T) {
	...
	Engine := new(consensus.FakeEngine)
	fullShardID := (env.clusterConfig.Quarkchain.Chains[0].ChainID << 16) | env.clusterConfig.Quarkchain.Chains[0].ShardSize | 0
	recoveredState, err := NewMinorBlockChain(env.db, nil, ethParams.TestChainConfig, env.clusterConfig, Engine, vm.Config{}, nil, fullShardID)
	checkErr(err)
...
func TestAddBlockReceiptRootNotMatch(t *testing.T) {
	...
	assert.Equal(t, errors.New("meta hash is not match"), err)
