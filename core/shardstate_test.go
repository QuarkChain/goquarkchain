...
func TestGetRootChainStakes(t *testing.T) {
	...
	contractAddr := env.clusterConfig.Quarkchain.GetRootChainPoSWContract()
	...
	applyTx := func(tx *types.Transaction, timestamp *uint64) bool {
		if timestamp == nil {
			...
			err = shardState.AddTx(tx)
			assert.NoError(t, err)
			block, receipt, err := shardState.FinalizeAndAddBlock(block)
			assert.NoError(t, err)
	...
	fmt.Println("balance before withdraw", balanceBefore)
	...
	//assert.True(t, applyTx(tx7, &realCreateTime))
