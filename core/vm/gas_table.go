...
func gasSStore(gt params.GasTable, evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	if true { // TODO:use Constantinople directly
...
func gasCall(gt params.GasTable, evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	fmt.Println("222--3", gas)
	memoryGas, err := memoryGasCost(mem, memorySize)
	if err != nil {
		return 0, err
	}
	fmt.Println("222-5", gas)
	var overflow bool
	if gas, overflow = math.SafeAdd(gas, memoryGas); overflow {
		return 0, errGasUintOverflow
	}
	fmt.Println("222-6", gas)
	evm.callGasTemp, err = callGas(gt, contract.Gas, gas, stack.Back(0))
	if err != nil {
		return 0, err
	}
	fmt.Println("222-7", gas)
	if gas, overflow = math.SafeAdd(gas, evm.callGasTemp); overflow {
		return 0, errGasUintOverflow
	}
	fmt.Println("222-8", gas)
	return gas, nil
}
...
func gasSuicide(gt params.GasTable, evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	if evm.ChainConfig().IsEIP150(evm.BlockNumber) {
		...
		if eip158 {
			...
			if evm.StateDB.Empty(address) && evm.StateDB.GetBalance(contract.Address(), evm.StateDB.GetQuarkChainConfig().GetDefaultChainTokenID()).Sign() != 0 {
