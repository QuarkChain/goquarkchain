...
func testMinorBlockChainImport(chain []types.IBlock, blockchain *MinorBlockChain) error {
	for _, block := range chain {
		...
		receipts, _, usedGas, err := blockchain.Processor().Process(block.(*types.MinorBlock), statedb, vm.Config{}, nil, nil)
		if err != nil {
...
func insertChain(done chan bool, blockchain *MinorBlockChain, chain []types.IBlock, t *testing.T) {
	...
	_, _, err := blockchain.InsertChain(chain, skip)
	if err != nil {
		fmt.Println(err)
		t.FailNow()
	}
...
// TestMinors that the insertion functions detect banned hashes.
func TestMinorBadHeaderHashes(t *testing.T) { testMinorBadHashes(t, false) }
func TestMinorBadBlockHashes(t *testing.T)  { testMinorBadHashes(t, true) }
...
// TestMinors that bad hashes are detected on boot, and the chain rolled back to a
// good state prior to the bad hash.
func TestMinorReorgBadHeaderHashes(t *testing.T) { testMinorReorgBadHashes(t, false) }
func TestMinorReorgBadBlockHashes(t *testing.T)  { testMinorReorgBadHashes(t, true) }

func testMinorReorgBadHashes(t *testing.T, full bool) {
	...
	headers := makeHeaderChain(blockchain.CurrentHeader().(*types.MinorBlockHeader), blockchain.CurrentBlock().Meta(), 4, engine, db, 10)
	...
	if full {
		...
		if _, err = blockchain.InsertHeaderChain(ToHeaders(headers), 1); err != nil {
	...
	blockchain.Stop()
	chainConfig := params.TestChainConfig
	// Create a new MinorBlockChain and check that it rolled back the state.
	ncm, err := NewMinorBlockChain(blockchain.db, nil, chainConfig, blockchain.clusterConfig, engine, vm.Config{}, nil, blockchain.fullShardID, nil)
	if err != nil {
	...
	if full {
		...
		if blocks[2].Header().GasLimit.Value.Uint64() != ncm.GasLimit() {
...
func TestMinorFastVsFullChains(t *testing.T) {
	...
	blocks, receipts := GenerateMinorBlockChain(chainConfig, clusterConfig.Quarkchain, genesis, engine, gendb, 1024, func(config *config.QuarkChainConfig, i int, block *MinorBlockGen) {
		...
		if i%3 == 2 {
			for j := 0; j < i%4+1; j++ {
				//	tx, err := types.SignTx(types.NewEvmTransaction(block.TxNonce(address), account.Recipient, big.NewInt(1000), params.TxGas, nil, 0, 0, 0, 0, nil), types.MakeSigner(0), key)
				//	if err != nil {
				//		panic(err)
				//	}
				//		block.AddTx(tx)
	...
	archive, _ := NewMinorBlockChain(archiveDb, nil, chainConfig, clusterConfig, engine, vm.Config{}, nil, clusterConfig.Quarkchain.Chains[0].ShardSize|0, nil)
	...
	fast, _ := NewMinorBlockChain(fastDb, nil, chainConfig, config.NewClusterConfig(), engine, vm.Config{}, nil, clusterConfig.Quarkchain.Chains[0].ShardSize|0, nil)
	defer fast.Stop()
...
func TestMinorLightVsFastVsFullChainHeads(t *testing.T) {
	...
	archive, _ := NewMinorBlockChain(archiveDb, nil, params.TestChainConfig, clusterConfig, engine, vm.Config{}, nil, config.NewClusterConfig().Quarkchain.Chains[0].ShardSize|0, nil)
	genesis, err := archive.InitGenesisState(rootBlock, genesis)
	if err != nil {
		panic(err)
	}
	...
	fast, _ := NewMinorBlockChain(fastDb, nil, params.TestChainConfig, clusterConfig, engine, vm.Config{}, nil, config.NewClusterConfig().Quarkchain.Chains[0].ShardSize|0, nil)
	...
	light, _ := NewMinorBlockChain(lightDb, nil, params.TestChainConfig, clusterConfig, engine, vm.Config{}, nil, config.NewClusterConfig().Quarkchain.Chains[0].ShardSize|0, nil)
...
func TestMinorChainTxReorgs(t *testing.T) {
	...
	postponed, _ := types.SignTx(types.NewEvmTransaction(0, addr1.Recipient, big.NewInt(1000), params.TxGas, nil, 0, 0, 3, 0, nil), signer, prvKey1)
	swapped, _ := types.SignTx(types.NewEvmTransaction(1, addr1.Recipient, big.NewInt(1000), params.TxGas, nil, 0, 0, 3, 0, nil), signer, prvKey1)
	...
	blockchain, _ := NewMinorBlockChain(db, nil, params.TestChainConfig, clusterConfig, engine, vm.Config{}, nil, clusterConfig.Quarkchain.Chains[0].ShardSize|0, nil)
	genesis, err = blockchain.InitGenesisState(rootBlock, genesis)
	if err != nil {
		panic(err)
	}
...
func TestMinorLogReorgs(t *testing.T) {
	...
	engine := &consensus.FakeEngine{}
	genesis := gspec.MustCommitMinorBlock(db, rootBlock, clusterConfig.Quarkchain.Chains[0].ShardSize|0)
	blockchain, _ := NewMinorBlockChain(db, nil, params.TestChainConfig, clusterConfig, engine, vm.Config{}, nil, config.NewClusterConfig().Quarkchain.Chains[0].ShardSize|0, nil)
	genesis, err = blockchain.InitGenesisState(rootBlock, genesis)
	if err != nil {
		panic(err)
	}
...
func TestMinorReorgSideEvent(t *testing.T) {
	...
	replacementBlocks, _ := GenerateMinorBlockChain(params.TestChainConfig, clusterConfig.Quarkchain, genesis, engine, db, 4, func(config *config.QuarkChainConfig, i int, gen *MinorBlockGen) {
		tx, err := types.SignTx(types.NewEvmContractCreation(gen.TxNonce(addr1.Recipient), new(big.Int), 1000000, new(big.Int), 0, 0, 3, 0, nil), signer, prvKey1)
		if i == 2 {
...
func TestMinorCanonicalBlockRetrieval(t *testing.T) {
	...
	for i := range chain {
		...
		if _, _, err := blockchain.InsertChain([]types.IBlock{chain[i]}, []bool{false}); err != nil {
