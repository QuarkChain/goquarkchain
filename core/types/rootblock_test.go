...
func TestRootBlockEncoding(t *testing.T) {
	...
	check("CoinbaseAmount", blockHeader.CoinbaseAmount.BalanceMap[TokenType{Key: 1}], new(big.Int).SetUint64(1))
	check("CoinbaseAmount", blockHeader.CoinbaseAmount.BalanceMap[TokenType{Key: 2}], new(big.Int).SetUint64(2))
