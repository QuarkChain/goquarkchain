...
func TestRootBlockEncoding(t *testing.T) {
	...
	if err := serialize.DeserializeWithTags(bb, &headers, serialize.Tags{ByteSize: 4}); err != nil {
		t.Fatal("Deserialize error: ", err)
	}

	bytes = make([]byte, 0, 0)
	err = serialize.SerializeWithTags(&bytes, headers, serialize.Tags{ByteSize: 4})
	if err != nil {
		t.Fatal("Serialize error: ", err)
	}
