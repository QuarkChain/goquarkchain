...
func WriteMinorBlock(db DatabaseWriter, block *types.MinorBlock) {
	...
	log.Debug(DBLOG, "Write MinorBlock branch", block.Header().Branch.Value, "height", block.NumberU64(), "hash", block.Hash().String(), "len(tx)", len(block.Transactions()))
...
func WriteRootBlock(db DatabaseWriter, block *types.RootBlock) {
	...
	log.Debug(DBLOG, "Write RootBlock height", block.NumberU64(), "hash", block.Hash())
	if err := db.Put(blockKey(block.Hash()), data); err != nil {
