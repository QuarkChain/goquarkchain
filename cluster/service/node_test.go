...
func TestAPIGather(t *testing.T) {
	...
	slvCfg := testSlaveConfig(0)
	_, err = slave.New(nil, slvCfg)
	services := map[string]struct {
		APIs  []rpc.API
		Maker InstrumentingWrapper
	}{
		...
		"slave API": {
			[]rpc.API{
				{
					Namespace: "rpc." + reflect.TypeOf(slave.SlaveServerSideOp{}).Name(),
					Service:   slave.NewServerSideOp(slvCfg.Id),
				},
			}, InstrumentedServiceMakerC,
		},
	}
	...
	// grpc api test of slave server
	endPoint := fmt.Sprintf("%s:%d", slvCfg.Ip, slvCfg.Port)
	rpCli := qrpc.NewRPCLient()
	rpcFuncs := rpCli.Getfuncs(0)
	for op, rpcFunc := range rpcFuncs {
		if res, err := rpCli.GetSlaveSideOp(endPoint, &qrpc.Request{Op: op}); err != nil {
			t.Errorf("failed to call %s function, error: %v", rpcFunc, err)
		} else if res.ErrorCode != 0 {
			t.Errorf("call %s function error, errorcode: %d", rpcFunc, res.ErrorCode)
		}
	}
}
