...
func newPeer(version int, p *p2p.Peer, rw p2p.MsgReadWriter) *QPeer {
	return &QPeer{
		Peer:             p,
		rw:               rw,
		version:          version,
...
func (p *QPeer) getRpcIdWithChan() (uint64, chan interface{}) {
	p.lock.Lock()
	defer p.lock.Unlock()
	p.rpcId = p.rpcId + 1
...
// peer.
func (p *QPeer) RootHead() *types.RootBlockHeader {
	p.lock.RLock()
...
func (p *QPeer) SetRootHead(rootTip *types.RootBlockHeader) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
// RootHead retrieves a copy of the current root head of the
// peer.
func (p *QPeer) MinorHead(branch uint32) *p2p.Tip {
	p.lock.RLock()
	defer p.lock.RUnlock()
...
// SetRootHead updates the root head of the peer.
func (p *QPeer) SetMinorHead(branch uint32, minorTip *p2p.Tip) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
func (p *QPeer) PeerID() string {
	return p.id
}

// SendTransactions sends transactions to the peer and includes the hashes
// in its transaction hash set for future reference.
func (p *QPeer) SendTransactions(branch uint32, txs []*types.Transaction) error {
...
func (p *QPeer) AsyncSendTransactions(branch uint32, txs []*types.Transaction) {
	select {
...
func (p *QPeer) SendNewTip(branch uint32, tip *p2p.Tip) error {
...
func (p *QPeer) AsyncSendNewTip(branch uint32, tip *p2p.Tip) {
	select {
...
func (p *QPeer) SendNewMinorBlock(branch uint32, block *types.MinorBlock) error {
...
func (p *QPeer) AsyncSendNewMinorBlock(branch uint32, block *types.MinorBlock) {
	select {
...
func (p *QPeer) getChan(rpcId uint64) chan interface{} {
	p.chanLock.Lock()
	defer p.chanLock.Unlock()
	return p.chans[rpcId]
}

func (p *QPeer) addChan(rpcId uint64, rpcchan chan interface{}) {
	p.chanLock.Lock()
	defer p.chanLock.Unlock()
	p.chans[rpcId] = rpcchan
}

func (p *QPeer) deleteChan(rpcId uint64) {
	p.chanLock.Lock()
	defer p.chanLock.Unlock()
...
func (p *QPeer) requestRootBlockHeaderList(rpcId uint64, hash common.Hash, amount uint32, reverse bool) error {
	if amount == 0 {
...
func (p *QPeer) GetRootBlockHeaderList(hash common.Hash, amount uint32, reverse bool) ([]*types.RootBlockHeader, error) {
...
func (p *QPeer) requestMinorBlockHeaderList(rpcId uint64, hash common.Hash, amount uint32, branch uint32, reverse bool) error {
...
func (p *QPeer) GetMinorBlockHeaderList(origin common.Hash, amount uint32, branch uint32, reverse bool) ([]*types.MinorBlockHeader, error) {
	rpcId, rpcchan := p.getRpcIdWithChan()
	defer p.deleteChan(rpcId)
...
// specified.
func (p *QPeer) requestRootBlockList(rpcId uint64, hashList []common.Hash) error {
...
func (p *QPeer) GetRootBlockList(hashes []common.Hash) ([]*types.RootBlock, error) {
	rpcId, rpcchan := p.getRpcIdWithChan()
	defer p.deleteChan(rpcId)
...
func (p *QPeer) requestMinorBlockList(rpcId uint64, hashList []common.Hash, branch uint32) error {
...
func (p *QPeer) GetMinorBlockList(hashes []common.Hash, branch uint32) ([]*types.MinorBlock, error) {
	rpcId, rpcchan := p.getRpcIdWithChan()
	defer p.deleteChan(rpcId)
...
func (p *QPeer) SendResponse(op p2p.P2PCommandOp, metadata p2p.Metadata, rpcId uint64, response interface{}) error {
...
func (p *QPeer) Handshake(protoVersion, networkId uint32, peerId common.Hash, peerPort uint16, rootBlockHeader *types.RootBlockHeader) error {
...
func (p *QPeer) readStatus(protoVersion, networkId uint32) (err error) {
...
// started.
func (ps *peerSet) Register(p *QPeer) error {
	ps.lock.Lock()
...
func (ps *peerSet) Peer(id string) *QPeer {
	ps.lock.RLock()
...
func (ps *peerSet) Peers() []*QPeer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()

	peers := make([]*QPeer, 0, len(ps.peers))
...
func (ps *peerSet) BestPeer() *QPeer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()
