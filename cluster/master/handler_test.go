...
func TestGetRootBlockHeaders(t *testing.T) {
	...
	for i, tt := range tests {
		...
		fmt.Printf("%d - %d", len(rheaders), len(headers))
		fmt.Printf("%d - %d - %d", rheaders[0].Number, headers[0].Number, pm.rootBlockChain.CurrentBlock().NumberU64())
		fmt.Printf("%v - %v - %v", rheaders[0].Hash(), headers[0].Hash(), pm.rootBlockChain.CurrentBlock().Hash())
