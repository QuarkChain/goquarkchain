...
// ── Bug 4 tests: CreateBlockToMine vs AddRootBlock concurrency ────────────────

// TestCreateBlockToMine_NoRaceDuringHeadTransition verifies that the race
// detector does not flag any data races when CreateBlockToMine reads rootTip
// and currentBlock while SetHead is concurrently modifying them.
//
// Bug 4 scenario: Before the fix, AddRootBlock updated m.rootTip, released the
// lock, then rewound m.currentBlock. CreateBlockToMine could read the new rootTip
// but the old currentBlock, causing isSameRootChain to receive mismatched heights
// and hit log.Crit ("should be same chain").
//
// The fix:
// 1. Extended m.mu lock scope in AddRootBlock to cover the entire root/minor
//    head transition (line 1087-1088: defer m.mu.Unlock())
// 2. Added m.mu.RLock in CreateBlockToMine to read a consistent snapshot
//
// Run with: go test -race ./cluster/shard/... -run TestCreateBlockToMine_NoRaceDuringHeadTransition
func TestCreateBlockToMine_NoRaceDuringHeadTransition(t *testing.T) {
	for i := 0; i < raceIterations; i++ {
		sb, db, stop := newTestShardBackend(t)
		...
		blocks, _ := core.GenerateMinorBlockChain(params.TestChainConfig, config.NewQuarkChainConfig(), genesis, engine, db, 5, nil)
		if _, err := sb.MinorBlockChain.InsertChain(toIBlocks(blocks), false); err != nil {
			stop()
			t.Fatalf("iter %d: InsertChain: %v", i, err)
		}

		var wg sync.WaitGroup
		wg.Add(2)

		// Goroutine A: CreateBlockToMine reads rootTip and currentBlock
		go func() {
			defer wg.Done()
			_, _ = sb.MinorBlockChain.CreateBlockToMine(nil, nil, nil, nil, nil)
		}()

		// Goroutine B: SetHead rewinds currentBlock (simulating part of AddRootBlock)
		go func() {
			defer wg.Done()
			targetHeight := genesisNum + 2
			if targetHeight < blocks[len(blocks)-1].NumberU64() {
				_ = sb.MinorBlockChain.SetHead(targetHeight)
			}
		}()
...
// TestCreateBlockToMine_ConsistentSnapshot verifies that CreateBlockToMine
// reads a consistent snapshot of rootTip and currentBlock.
//
// Run with: go test -race ./cluster/shard/... -run TestCreateBlockToMine_ConsistentSnapshot
func TestCreateBlockToMine_ConsistentSnapshot(t *testing.T) {
	sb, db, stop := newTestShardBackend(t)
	defer stop()
	...
	blocks, _ := core.GenerateMinorBlockChain(params.TestChainConfig, config.NewQuarkChainConfig(), genesis, engine, db, 3, nil)
	if _, err := sb.MinorBlockChain.InsertChain(toIBlocks(blocks), false); err != nil {
		t.Fatalf("InsertChain: %v", err)
	}

	var wg sync.WaitGroup
	const numCreateWorkers = 20
	const numSetHeadWorkers = 10

	for i := 0; i < numCreateWorkers; i++ {
		wg.Add(1)
		go func() {
			defer wg.Done()
			_, err := sb.MinorBlockChain.CreateBlockToMine(nil, nil, nil, nil, nil)
			_ = err
		}()
	}

	for i := 0; i < numSetHeadWorkers; i++ {
		wg.Add(1)
		go func(idx int) {
			defer wg.Done()
			targetHeight := genesis.NumberU64() + uint64(idx%3) + 1
			_ = sb.MinorBlockChain.SetHead(targetHeight)
		}(i)
	}
...
// ── Bug 1 fix verification: HasBlock and WriteBlockWithState ─────────────────
...
//
// Before the fix, WriteBlockWithState called CommitMinorBlockByHash internally.
// The fix removes that call so the commit marker is written ONLY by the shard
// layer after distributed coordination (broadcast + report to master) completes.
func TestInsertChain_BodyWrittenWithoutCommitMarker(t *testing.T) {
	...
	if !sb.MinorBlockChain.HasBlock(block.Hash()) {
		t.Error("HasBlock should return true after InsertChain — block body must be in DB")
	}
...
//
// This is the core Bug 1 fix: HasBlock previously returned
// IsMinorBlockCommittedByHash, which could return true for a block whose body
// had been deleted by setHead. Now HasBlock reads the body from the DB directly
// (rawdb.ReadMinorBlock), independent of the commit marker.
func TestHasBlock_FalseWhenBodyAbsentButMarkerPresent(t *testing.T) {
	...
	if sb.MinorBlockChain.HasBlock(fakeHash) {
		t.Error("HasBlock must return false when block body is absent — " +
			"HasBlock must not rely on the commit marker (Bug 1 regression)")
	}
}
