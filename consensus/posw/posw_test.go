...
func TestPoSWCoinbaseAddrsCntByDiffLen(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		...
		newBlock = tip.CreateBlockToAppend(nil, nil, &randomAcc, nil, nil, nil, nil, nil)
...
func TestPoSWCoinBaseSendUnderLimit(t *testing.T) {
	...
	tip := blockchain.GetMinorBlock(blockchain.CurrentHeader().Hash())
	newBlock := tip.CreateBlockToAppend(nil, nil, &acc1, nil, nil, nil, nil, nil)
	newBlock, _, err = blockchain.FinalizeAndAddBlock(newBlock)
	if err != nil {
		t.Fatalf("failed to FinalizeAndAddBlock: %v", err)
	}

	//fmt.Println("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAa",blockchain.CurrentBlock().NumberU64(),blockchain.CurrentBlock().Meta().Root.String())
	//s1,err:=blockchain.State()
	//fmt.Println("s1",s1.GetBalances(acc1.Recipient))
	//s2,err:=blockchain.StateAt(blockchain.CurrentBlock().Meta().Root)
	//fmt.Println("s1",s2.GetBalances(acc1.Recipient))
	...
	balance := evmState.GetBalance(acc1.Recipient, 0)
	...
	sb, err := blockchain.State()
	fmt.Println("err", err, sb.GetBalances(acc1.Recipient))
	...
	if mb, err = blockchain.CreateBlockToMine(nil, &acc2, nil, nil, nil); err != nil {
	...
	fmt.Println("!!!!!!!!!!!", len(mb.Transactions()))
	blc := make(map[uint64]*big.Int)
	...
	if balanceExp1.Cmp(blc[qkcCommon.TokenIDEncode("QKC")]) != 0 {
		t.Errorf("Balance: expected %v, got %v", balanceExp1, blc[qkcCommon.TokenIDEncode("QKC")])
	}
	...
	tb1 := make(map[uint64]*big.Int)
	if tb1, err = blockchain.GetBalance(acc1.Recipient, nil); err != nil {
		t.Fatalf("get balance error %v", err)
	}
	if tb1[qkcCommon.TokenIDEncode("QKC")].Cmp(balanceExp1) != 0 {
		t.Errorf("Balance: expected %v, got %v", balanceExp1, tb1)
	}
	tb2 := make(map[uint64]*big.Int)
	if tb2, err = blockchain.GetBalance(acc2.Recipient, nil); err != nil {
		t.Fatalf("get balance error %v", err)
	} else if tb2[qkcCommon.TokenIDEncode("QKC")].Cmp(balanceExp) != 0 { //acc2 only mined 1 block successfully
...
func TestPoSWCoinbaseSendEqualLocked(t *testing.T) {
	...
	if minorBlock, err := blockchain.CreateBlockToMine(nil, &acc1, nil, nil, nil); err != nil {
	...
	if evmState, err := blockchain.State(); err != nil {
		...
		balance := evmState.GetBalance(acc1.Recipient, qkcCommon.TokenIDEncode("QKC"))
	...
	if tb1, err = blockchain.GetBalance(acc1.Recipient, nil); err != nil {
		...
		if tb1[qkcCommon.TokenIDEncode("QKC")].Cmp(balanceExp) != 0 {
...
func TestPoSWValidateMinorBlockSeal(t *testing.T) {
	...
	if balance, err := blockchain.GetBalance(reci, nil); err != nil {
		...
		if balanceExp.Cmp(balance[qkcCommon.TokenIDEncode("QKC")]) != 0 {
			t.Errorf("balance: expected: %v, got %v", balanceExp, balance)
		}
	}
	...
	tip := blockchain.GetMinorBlock(blockchain.CurrentHeader().Hash())
	newBlock := tip.CreateBlockToAppend(nil, diff, &genesis, nil, nil, nil, nil, nil)
