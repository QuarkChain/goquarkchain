...
func (s *stTransaction) UnmarshalJSON(input []byte) error {
	type stTransaction struct {
		...
		Value           []string              `json:"value"`
		PrivateKey      *hexutil.Bytes        `json:"secretKey"`
		TransferTokenId []math.HexOrDecimal64 `json:"transferTokenId"`
	}
	...
	if dec.TransferTokenId != nil {
		s.TransferTokenID = make([]uint64, len(dec.TransferTokenId))
		for k, v := range dec.TransferTokenId {
			s.TransferTokenID[k] = uint64(v)
		}
	}
	if dec.Value != nil {
		s.Value = dec.Value
